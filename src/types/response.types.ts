...
export function SuccessResponse<T>(data: T | null = null, message?: string, status: number = 200): ResponseModel<T> {
  return {
    status,
    success: true,
    message,
...
export function ErrorResponse(message: string, status: number = 500): ResponseModel<{}> {
  return {
    status,
    success: false,
    message,
