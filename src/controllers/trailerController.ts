...
export const createTrailer = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    const userId = req.user.userId
    const { year, make, vin, licencePlate, state, nrOfAxles, unitNumber, length, type } = req.body as TrailerPayload
    ...
    const trailerExists = await Trailer.findOne({ vin })
    if (trailerExists) return next(new ErrorHandler('Trailer already exists', 400))
    ...
    res.status(201).json(SuccessResponse(trailer, 'Trailer created successfully'))
  },
)
...
export const updateTrailer = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const { year, make, vin, licencePlate, state, nrOfAxles, unitNumber } = req.body as TrailerPayload

    if (!licencePlate) return next(new ErrorHandler('Licence plate is required', 400))
    if (!unitNumber) return next(new ErrorHandler('Unit number is required', 400))

    const existingTrailer = await Trailer.findOne({ _id: trailerId, userId: req.user.userId })
    if (!existingTrailer) return next(new ErrorHandler('Trailer not found', 404))
    ...
    res.status(200).json(SuccessResponse(updatedTrailer, 'Trailer updated successfully'))
  },
)
...
export const deleteTrailer = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    res.status(200).json(SuccessResponse(null, 'Trailer deleted successfully'))
  },
)

export const getTrailer = CatchAsyncErrors(async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  const { trailerId } = req.params
  const userId = req.user.userId
  ...
  res.status(200).json(SuccessResponse(trailer, 'Trailer fetched successfully'))
})
...
export const uploadTrailerFile = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (!updatedTrailer) return next(new ErrorHandler('Failed to update trailer', 500))

      res.status(200).json(SuccessResponse(updatedTrailer, 'File uploaded successfully'))
    } catch (error: any) {
...
export const downloadTrailerFile = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (!trailer) return next(new ErrorHandler('File not found or access denied', 404))

    const fileData = trailer.files.find((file: any) => file.filename === filename)
...
export const deleteTrailerFile = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      await Trailer.updateOne({ userId }, { $pull: { files: { filename } } })
