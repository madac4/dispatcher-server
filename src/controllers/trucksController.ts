...
export const createTruck = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		const userId = req.user.userId
		const { year, make, vin, licencePlate, state, nrOfAxles, unitNumber } =
			req.body as TruckPayload
		...
		res.status(201).json(
			SuccessResponse(truck, 'Truck created successfully'),
		)
	},
)

export const updateTruck = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		res.status(200).json(
			SuccessResponse(updatedTruck, 'Truck updated successfully'),
		)
	},
)

export const deleteTruck = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		const { truckId } = req.params
		const userId = req.user.userId
		...
		res.status(200).json(
			SuccessResponse(null, 'Truck deleted successfully'),
		)
	},
)

export const getTruck = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		const { truckId } = req.params
		const userId = req.user.userId
		...
		res.status(200).json(
			SuccessResponse(truck, 'Truck fetched successfully'),
		)
	},
)
...
export const uploadTruckFile = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		try {
			...
			if (!updatedTruck)
				return next(new ErrorHandler('Failed to update truck', 500))

			res.status(200).json(
				SuccessResponse(updatedTruck, 'File uploaded successfully'),
			)
		} catch (error: any) {
			return next(
...
export const downloadTruckFile = CatchAsyncErrors(
	async (req: Request, res: Response, next: NextFunction): Promise<void> => {
		...
		const fileData = truck.files.find(
