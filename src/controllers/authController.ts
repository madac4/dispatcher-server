...
export const register = CatchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
  ...
  await Team.findByIdAndUpdate(invitation.teamId, { $addToSet: { members: user._id } })
...
export const login = CatchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
  ...
  res.status(200).json(SuccessResponse<LoginResponse>({ accessToken, refreshToken }, 'Login successful'))
})
...
export const refreshToken = CatchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    decoded = jwt.verify(refreshToken, process.env.JWT_SECRET as string) as { userId: string; role: UserRole }
  } catch (error) {
    await tokenDoc.deleteOne()
  ...
  res.status(200).json(SuccessResponse<{ accessToken: string }>({ accessToken }, 'Refresh token successful'))
})
...
export const resetPassword = CatchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!token || !password || !confirmPassword) return next(new ErrorHandler('All fields are required', 400))
  ...
  const isMatch = await user.comparePassword(password)
  if (isMatch) return next(new ErrorHandler('New password cannot be the same as the old password', 400))
...
export const updatePassword = CatchAsyncErrors(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!currentPassword || !password || !confirmPassword) return next(new ErrorHandler('All fields are required', 400))
  if (password !== confirmPassword) return next(new ErrorHandler('Passwords do not match', 400))
  ...
  if (isSamePassword) return next(new ErrorHandler('New password cannot be the same as the old password', 400))
