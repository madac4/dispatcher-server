...
import { ModeratorOrderDTO, OrderDTO, PaginatedOrderDTO } from '../dto/order.dto';
...
export const createOrder = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!settings)
      return next(new ErrorHandler('Please complete your company information in settings', 404));
    ...
    const newOrder = new Order({
      ...
      stops:
        typeof orderData.stops === 'string' ? JSON.parse(orderData.stops) : orderData.stops || [],
      files: [],
      status: 'pending',
      axleConfigs:
    ...
    if (orderData.messages) {
      const messages = (
        typeof orderData.messages === 'string' ? JSON.parse(orderData.messages) : orderData.messages
      ) as string[];

      messages.forEach(async (message) => {
        await ChatService.sendUserMessage(savedOrder._id, message, userId);
      });
    }
...
export const duplicateOrder = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    res.status(201).json(SuccessResponse(new OrderDTO(newOrder), 'Order duplicated successfully'));
  }
);
...
export const getStatuses = CatchAsyncErrors(async (req: Request, res: Response) => {
...
export const getOrderByNumber = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const order = await Order.findOne(query).populate('truckId').populate('trailerId');

    if (!order) return next(new ErrorHandler('Order not found', 404));

    if (user.role !== UserRole.USER) {
      const userSettings = await Settings.findOne({ userId: order.userId });
      if (!userSettings) return next(new ErrorHandler('User settings not found', 404));

      const orderDTO = new ModeratorOrderDTO(order, userSettings);

      return res.status(200).json(SuccessResponse(orderDTO, 'Order retrieved successfully'));
    }

    const orderDTO = new OrderDTO(order);

    res.status(200).json(SuccessResponse(orderDTO, 'Order retrieved successfully'));
  }
);
...
export const uploadOrderFile = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const query =
      user.role === UserRole.USER ? { _id: orderId, userId: user.userId } : { _id: orderId };
...
export const updateOrderStatus = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    res
      .status(200)
      .json(SuccessResponse(order, `Order status updated successfully to ${formatStatus(status)}`));
  }
);
