...
import { formatStatus, ICreateOrderRequest, IOrder, OrderStatus } from '../types/order.types'
import {
  ...
  try {
    ...
    if (files && files.length > 0) {
      ...
      if (uploadedFiles.length > 0) {
        await Order.findByIdAndUpdate(savedOrder._id, { $push: { files: { $each: uploadedFiles } } }, { new: true })
      }
    }
    ...
    return next(new ErrorHandler('Internal server error while creating order', 500))
  }
}
...
export const getOrders = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const orderDtos = orders.map(order => new PaginatedOrderDTO(order as IOrder))
...
export const getStatuses = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (type === 'active') {
      statuses = [OrderStatus.ACTIVE, OrderStatus.PENDING, OrderStatus.PROCESSING]
    } else if (type === 'completed') {
      statuses = [OrderStatus.REQUIRES_INVOICE, OrderStatus.REQUIRES_CHARGE]
    } else if (type === 'paid') {
    ...
    const response = await Promise.all([
      {
        value: 'all',
        label: 'All Statuses',
        quantity: await Order.countDocuments({ userId: req.user.userId, status: { $in: statuses } }),
      },
    ...
    res.status(200).json(SuccessResponse(response, 'Statuses retrieved successfully'))
  } catch (error) {
...
export const getOrderByNumber = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!orderNumber) return next(new ErrorHandler('Order number is required', 400))

    const order = await Order.findOne({ orderNumber, userId }).populate('truckId').populate('trailerId')

    if (!order) return next(new ErrorHandler('Order not found', 404))

    res.status(200).json(SuccessResponse(new OrderDTO(order), 'Order retrieved successfully'))
  } catch (error) {
    console.error('Error getting order by number:', error)
    return next(new ErrorHandler('Internal server error while retrieving order', 500))
  }
}
...
export const uploadOrderFile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const file = req.file
    ...
    if (!updatedOrder) return next(new ErrorHandler('Failed to update order', 500))

    res.status(200).json(SuccessResponse(updatedOrder, 'File uploaded successfully'))
  } catch (error: any) {
...
export const getOrderFiles = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { orderId } = req.params
    const userId = req.user.userId
    ...
    return next(new ErrorHandler('Internal server error while fetching files', 500))
  }
}

export const downloadOrderFile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (!order) return next(new ErrorHandler('File not found or access denied', 404))

    const fileData = order.files.find((file: any) => file.filename === filename)
...
export const deleteOrderFile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { filename } = req.params
    const userId = req.user.userId
