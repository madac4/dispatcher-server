...
export const sendMessage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const populatedMessage = await ChatMessage.findById(savedMessage._id).populate('userId', 'firstName lastName email')
    ...
    res.status(201).json(SuccessResponse(populatedMessage, 'Message sent successfully'))
  } catch (error) {
    console.error('Error sending message:', error)
    return next(new ErrorHandler('Internal server error while sending message', 500))
  }
}

// Get chat messages for an order
export const getOrderMessages = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const userId = req.user.id
    if (!userId) return next(new ErrorHandler('User not authenticated', 401))
    ...
    if (unreadMessages.length > 0) {
      await ChatMessage.updateMany({ _id: { $in: unreadMessages.map(msg => msg._id) } }, { isRead: true })
    }

    // Update unread count in order chat
    await OrderChat.findOneAndUpdate({ orderId }, { unreadCount: 0 }, { upsert: true })
    ...
    return next(new ErrorHandler('Internal server error while retrieving messages', 500))
  }
}

// Get all order chats for a user
export const getUserOrderChats = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const userId = req.user.id
    if (!userId) return next(new ErrorHandler('User not authenticated', 401))
    ...
    const orders = await Order.find({ userId }).sort({ updatedAt: -1 }).skip(skip).limit(limit).lean()
    ...
    return next(new ErrorHandler('Internal server error while retrieving order chats', 500))
  }
}
...
export const markMessagesAsRead = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    await ChatMessage.updateMany({ orderId, isRead: false, userId: { $ne: userId } }, { isRead: true })
    ...
    return next(new ErrorHandler('Internal server error while marking messages as read', 500))
  }
}
  ...
  try {
    ...
    if (orderChat) {
      ...
      if (orderChat.lastMessage?.toString() === messageId) {
        const lastMessage = await ChatMessage.findOne({ orderId: message.orderId }).sort({ createdAt: -1 })
    ...
    return next(new ErrorHandler('Internal server error while deleting message', 500))
  }
}
...
export const getUnreadCount = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    res.status(200).json(SuccessResponse({ unreadCount }, 'Unread count retrieved'))
  } catch (error) {
    console.error('Error getting unread count:', error)
    return next(new ErrorHandler('Internal server error while getting unread count', 500))
  }
}
